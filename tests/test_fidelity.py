...
class TestFidelityPositions(unittest.TestCase):
    def setUp(self) -> None:
        self.positions = list(
            fidelity.FidelityAccount(
                positions=Path("tests/fidelity_positions.csv")
            ).positions()
        )
    ...
    def test_tBill(self) -> None:
        self.assertEqual(self.positions[0].instrument, Bond("942792RU5", Currency.USD))
    ...
    def test_aapl(self) -> None:
        self.assertEqual(self.positions[1].instrument, Stock("AAPL", Currency.USD))
    ...
    def test_robo(self) -> None:
        self.assertEqual(self.positions[2].instrument, Stock("ROBO", Currency.USD))
    ...
    def test_v(self) -> None:
        self.assertEqual(self.positions[5].instrument, Stock("V", Currency.USD))
...
class TestFidelityTransactions(unittest.TestCase):
    def setUp(self) -> None:
        self.activity = list(
            fidelity.FidelityAccount(
                transactions=Path("tests/fidelity_transactions.csv")
            ).activity()
        )
        self.activity.sort(key=lambda t: t.date)

        self.activityByDate = {
            d: list(t) for d, t in groupby(self.activity, key=lambda t: t.date.date())
        }
    ...
    def test_buySecurity(self) -> None:
        ...
        self.assertEqual(
            ...
            Trade(
                ...
                amount=Cash(currency=Currency.USD, quantity=Decimal("-5427.15")),
    ...
    def test_buyToOpenOption(self) -> None:
        ...
        self.assertEqual(
            ...
            Trade(
                ...
                quantity=Decimal("32"),
                amount=Cash(currency=Currency.USD, quantity=Decimal("-3185.67")),
...
class TestFidelityBalance(unittest.TestCase):
    ...
    def testUSDBalance(self) -> None:
        self.assertEqual(
            self.balance.cash, {Currency.USD: helpers.cashUSD(Decimal("15678.89"))}
        )
